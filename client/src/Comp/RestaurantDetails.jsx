...
export default function RestaurantDetails() {
  ...
  // حالات التحكم في صناديق التعديل والحذف (نفس ستايل الهوتيل)
...
<Card style={styles.mainCard}>
<CardBody style={styles.cardBody}>
<CardTitle tag="h2" style={styles.title}>{restaurant.name}</CardTitle>
<p style={styles.subtitle}>

                  A popular fast-food restaurant known for its crispy fried chicken and unique blend of spices.
</p>
<div style={styles.infoRow}>
<span style={styles.pill}>⭐ {restaurant.averageRating || 0} / 5</span>
<span style={styles.pill}>👥 {reviews.length} reviews</span>
<span style={styles.pill}>📍 {restaurant.location?.name}</span>
</div>
<CardText style={styles.description}>{restaurant.description}</CardText>
...
<iframe
                  ...
                  src={`https://www.google.com/maps?q=${restaurant.location?.lat},${restaurant.location?.lng}&z=16&output=e…
...
<div className="text-center mt-4">
<Button style={styles.backButton} onClick={() => navigate("/restaurants")}>← Back to Restaurants</Button>
</div>
</CardBody>
</Card>
</Col>
